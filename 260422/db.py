...
class MyDB:
    ...
    def commit(self):
        try:
            ...
            #문제가 발생하는 이유는? -> 서버와의 연결이 되지 않은 경우(self.db_server에 데이터가 없거나 아예 존재하지 않을 경우)
            print('데이터베이스 서버와의 연결이 되어있지 않습니다. sql_query() 함수를 호출하여 서버와의 연결을 해주세요')
    
    def sql_query(self,query,*datas):
        #query : sql query문이 입력되는 매개변수
        #datas : query문에서 사용이 될 데이터의 목록

        #문제점 : 서버의 재접속으로 commit전의 데이터가 날아감.
        #이미 접속중인 경우 재접속 금지(2026.04.20 update)
        #해결방법 : self.db_server에 데이터가 존재한다면 서버에 접속 중이다.
        try:
            self.db_server
            #변수가 존재하지 않으면 NameError 발생
            print('접속된 서버가 존재함')
        except:
            #예외상황이 발생하면 서버와 연결(self.db_server 라는 변수가 없을 시 발생)
            #DB_server와의 연결
        ...
        #self.변수명 // 변수명 의 차이는?
            #self.변수명 : 독립적으로 객체안에 저장이 되는 변수(함수 호출 후에도 데이터가 존재)
        ...
        try:
            ...
            #query가 select문이라면?
            #select * from table, """  select * from table  """ // SELECT * FROM TABLE -> 두 가지 경우 모두 참
            #좌측의 공백 제거 , 데소문자 통일, 시작값이 select와 같은가 startswith()
